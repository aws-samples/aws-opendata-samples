...
1. Navigate to your **dataset bucket** (not logging bucket created in Step 1) in the S3 Console.
...
Refer to [official documentation](https://docs.aws.amazon.com/AmazonS3/latest/userguide/enable-server-access-logging.html) for more detailed steps and information about the different configuration options.
...
6. Set **Days after object creation** to `30` to delete logs 30 days after they are created.
   > **Note:**  
   > To remain eligible for Open Data sponsorship, the maximum retention period for access logs is **30 days**.  
   > Setting a longer retention period may result in ineligibility for the program.
...
2. Wait several hours. 
   > **Note:**  
   > S3 server access logging uses best-effort delivery. While logs are typically delivered within a few hours, delays, duplication, or occasional loss of log records may occur.  
   > This logging mechanism is not designed to provide a complete accounting of all requests.  
   > For details, see the [S3 Server Access Logging documentation](https://docs.aws.amazon.com/AmazonS3/latest/userguide/ServerLogs.html#LogDeliveryBestEffort).
3. Open the **logging bucket** (created in Step 1) and locate recent log files.
4. Confirm the log files contain details that correspond to your test requests, such as request time, requester’s IP, requested object, and response code.  
   > **Note:**  
   > To view example server access logs, see the [S3 Log Format documentation](https://docs.aws.amazon.com/AmazonS3/latest/userguide/LogFormat.html).  
   > For a breakdown of each log record field, refer to the [Log Record Fields reference](https://docs.aws.amazon.com/AmazonS3/latest/userguide/LogFormat.html#log-record-fields).
...
> ⚠️ **Important**: All analysis of access logs must be performed outside of your sponsored Open Data account. Running compute workloads, such as Athena queries in your sponsored AWS account, is not permitted under the program’s terms.
...
![View Additional Charts Option](./images/monitoring-Amazon-S3-dataset-usage-with-server-access-logs-and-CloudWatch-Metrics/view-addition-charts.png)
_Fig 1. Select the View addition charts option metrics page._
...
_Fig 2. Select the Storage metrics tab on the Bucket metrics page._
...
_Fig 3. CloudWatch charts showing total bucket size and object count over two weeks._
...
_Fig 4. Select the Request metrics tab on the Bucket metrics page._
...
![CloudWatch Request Metrics Chart Example](./images/monitoring-Amazon-S3-dataset-usage-with-server-access-logs-and-CloudWatch-Metrics/ex-request-metrics.png)

_Fig 5. CloudWatch charts showing All requests and Get requests metrics over two weeks._
